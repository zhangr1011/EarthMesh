...
MODULE MOD_utilities
   ...
   SUBROUTINE cellwidth_save(inputfile, num_dbx, wp, cellwidth)

      IMPLICIT NONE
      character(pathlen), intent(in)  :: inputfile
      integer, intent(in) :: num_dbx
      real(r8), allocatable, intent(in) :: wp(:,:), cellwidth(:)
      integer :: ncid, dimID_dbx, varid(3)
 
      CALL CHECK(NF90_CREATE(trim(inputfile), ior(nf90_clobber, nf90_netcdf4), ncid))
      CALL CHECK(NF90_DEF_DIM(ncid, "num_dbx", num_dbx, DimID_dbx))
      CALL CHECK(NF90_DEF_VAR(ncid, "lonw", NF90_DOUBLE, (/ DimID_dbx /), varid(1)))
      CALL CHECK(NF90_DEF_VAR(ncid, "latw", NF90_DOUBLE, (/ DimID_dbx /), varid(2)))
      CALL CHECK(NF90_DEF_VAR(ncid, "cellwidth", NF90_DOUBLE, (/ DimID_dbx /), varid(3)))
      CALL CHECK(NF90_ENDDEF(ncid))
      CALL CHECK(NF90_PUT_VAR(ncid, varid(1), wp(:,1)))
      CALL CHECK(NF90_PUT_VAR(ncid, varid(2), wp(:,2)))
      CALL CHECK(NF90_PUT_VAR(ncid, varid(3), cellwidth))
      CALL CHECK(NF90_CLOSE(ncid))
      
   END SUBROUTINE cellwidth_save

   SUBROUTINE cellwidth_read(inputfile, cellwidth)

      IMPLICIT NONE
      character(pathlen), intent(in) :: inputfile
      integer :: ncid, dimID_dbx, varid, num_dbx
      real(r8), dimension(:), allocatable, intent(out) :: cellwidth

      CALL CHECK(NF90_OPEN(trim(inputfile), nf90_nowrite, ncid))! 1. NF90_OPEN 打开文件 
      CALL CHECK(NF90_INQ_DIMID(ncid, "num_dbx", dimID_dbx))! 2. NF90_INQ_DIMID获取维度ID 
      CALL CHECK(NF90_INQUIRE_DIMENSION(ncid, dimID_dbx, len = num_dbx))! 3. NF90_INQUIRE_DIMENSION获取各维度的长度
      allocate(cellwidth(num_dbx))
      CALL CHECK(NF90_INQ_VARID(ncid, 'cellwidth', varid))
      CALL CHECK(NF90_GET_VAR(ncid, varid, cellwidth))
      CALL CHECK(NF90_CLOSE(ncid))! 7. NF90_CLOSE关闭文件
   
   END SUBROUTINE cellwidth_read
   ...
   SUBROUTINE MPAS_Mesh_Save(lndname, num_sjx, num_dbx, num_edge, latCell, lonCell, xCell, yCell, zCell, &
      ...
      allocate(indexToCellID(num_dbx-1));   indexToCellID = [(i, i=1, num_dbx-1)]
      allocate(indexToVertexID(num_sjx-1)); indexToVertexID = [(i, i=1, num_sjx-1)]
      allocate(indexToEdgeID(num_edge-1));   indexToEdgeID = [(i, i=1, num_edge-1)]
      ...
      CALL CHECK(nf90_put_att(ncid, NF90_GLOBAL, "x_period", 0.0_r8))
      CALL CHECK(nf90_put_att(ncid, NF90_GLOBAL, "y_period", 0.0_r8))
